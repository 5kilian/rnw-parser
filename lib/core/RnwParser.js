...
function RnwParser() {
    ...
    this.parseRnwRow = (rawRow) => {
        ...
        switch (figure.type) {
            case 'PlaceFigure':
            case 'TransitionFigure':
                figure.position = { x: parseInt(rawRow[3+rawRow[3]*3+1]), y: parseInt(rawRow[3+rawRow[3]*3+2]) };
                figure.width = parseInt(rawRow[3+rawRow[3]*3+3]);
                figure.height = parseInt(rawRow[3+rawRow[3]*3+4]);
                break;
            case 'ArcConnection':
                figure.positions = [];
                for (let i=1; i<=rawRow[3+rawRow[3]*3+1]; i++) {
                    figure.positions.push({ x: parseInt(rawRow[3+rawRow[3]*3+i*2]), y: parseInt(rawRow[3+rawRow[3]*3+i*2+1])});
                }
        }
