...
function RnwParser() {
    ...
    function parseRnwRow(rawRow) {
        ...
        switch (figure.type) {
            ...
            case 'CPNText':
            case 'Declaration':
                delete figure.FrameColor;
                delete figure.FillColor;
        ...
        if (figure !== undefined) {
            (figure instanceof Array ? figure : [figure]).forEach(raw => {
                figures.push({
                    ...
                    width: parseInt(raw.graphics.dimension.x),
                    height: parseInt(raw.graphics.dimension.y),
                    FillColor: parseColor(raw.graphics.fill.color),
                    FrameColor: parseColor(raw.graphics.line.color)
                });
                if (raw.initialMarking !== undefined || raw.name !== undefined) {
                    let textType = raw.name === undefined ? 'initialMarking' : 'name';
                    figures.push({
                        type: 'Text',
                        position: {
                            x: parseInt(raw.graphics.position.x) + parseInt(raw[textType].graphics.offset.x),
                            y: parseInt(raw.graphics.position.y) + parseInt(raw[textType].graphics.offset.y)
                        },
    ...
    function getArcs(arc) {
        let figures = [];
        if (arc !== undefined) {
            (arc instanceof Array ? arc : [arc]).forEach(raw => {
                figures.push({
                    id: parseInt(raw.id),
                    type: 'Arc',
                    source: parseInt(raw.source),
                    target: parseInt(raw.target),
                    FrameColor: parseColor(raw.graphics.line.color),
                    LineStyle: raw.graphics.line.style,
                    tip: getTip(raw.type.text)
                });
            })
        }
        return figures;
