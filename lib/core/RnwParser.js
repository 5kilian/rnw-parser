let fs = require('fs');
let pnml = require('fast-xml-parser');
...
function RnwParser() {
    ...
    this.parsePnml = (xml) => {
        let data = pnml.parse(xml, { attrPrefix : '', ignoreNonTextNodeAttr : false, ignoreTextNodeAttr : false }).pnml;

        let ref = {};
        let figures = []
            .concat(getPlaces(data.net.place, ref))
            .concat(getTransitions(data.net.transition, ref))
            .concat(getToolSpecific(data.net.toolspecific, ref))
            .concat(getArcs(data.net.arc, ref));
